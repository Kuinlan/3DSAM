...
def anchor_index_padding(data, anchor_num, index, anchor_num_set, 
                         pad_num_min, generator, is_training):
    ...
    anchor_num_cumsum = torch.cat(
        ...
        device=anchor_num.device), anchor_num.cumsum(0)], 
        dim=0
    )
        ...
        for n in range(N):
            ...
            if need_pad[n]:
                ...
                sample_index = (torch.randperm(
                    anchor_num[n], generator=generator,
                    dtype=torch.int64, device=device) + low[n])[:anchor_num_set-pad_num_min]
                i_ids = torch.cat([i_ids, i_ids_get[sample_index]])
                j_ids = torch.cat([j_ids, j_ids_get[sample_index]])

            ids_shuffle = torch.randperm(
                len(i_ids), generator=generator, 
