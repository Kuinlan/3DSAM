...
def anchor_index_padding(data, mask, anchor_num_max, is_training=False):
    ...
    if is_training:
        pad_num = torch.where(
            anchor_num < anchor_num_max,
            anchor_num_max - anchor_num,
            0,
        )  # (N', )
        ...
        for n in range(N):
            ...
            anc_i_ids[n, :pad_num[n]] = anc_i_gt[n, :pad_num[n]]
            anc_j_ids[n, :pad_num[n]] = anc_j_gt[n, :pad_num[n]]

            sample = torch.randperm(anchor_num[n], dtype=torch.int64, device=device) + low

            anc_i_ids[n, pad_num[n]:] = i_ids[sample[:anchor_num_max-pad_num[n]]]
            anc_j_ids[n, pad_num[n]:] = j_ids[sample[:anchor_num_max-pad_num[n]]]
            
        sample_replacement = torch.randint(low=0, high=anchor_num_max, size=(anchor_num_max, ), dtype=torch.int64, device=device)
        anc_i_ids = anc_i_ids[:, sample_replacement]
        anc_j_ids = anc_j_ids[:, sample_replacement]
